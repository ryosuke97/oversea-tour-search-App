...
export class AppComponent implements OnInit {
  ...
  bookmarks; // ブックマーク
  isMobile = false; // PCとSPの判定
  MOBILE_SCREEN_WIDTH = 768; // SP判定画面幅
  isCollapsed = false; // PCとモバイルの判定
  areas = [ // 3エリアの全データ
  ...
  public constructor(private httpService: HttpService, viewContainerRef: ViewContainerRef) { // HttpServiceのDI
  ...
  getTour() {
    ...
    for (let i = 0; i < this.areas.length; i++) {
      ...
      this.httpService.getTourData(areaCode).subscribe(
        ...
        error => alert("通信エラー¥n" + error) // 通信に失敗した場合
      );
    }
  }
  // 受信データの取得
